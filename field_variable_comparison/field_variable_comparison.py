...
def merged_field_variable_comparison(dataset1, dataset2, field_name):
    ...
    # Get all cell point ids at once
    cell1_point_ids = [dataset1.get_cell(i).point_ids for i in range(len(indices))]
    cell2_point_ids = [dataset2.get_cell(indices[i]).point_ids for i in range(len(indices))]

    # Flatten and stack for vectorized access
    vals1 = np.concatenate([dataset1.point_data[field_name][ids] for ids in cell1_point_ids])
    vals2 = np.concatenate([dataset2.point_data[field_name][ids] for ids in cell2_point_ids])
